...
public class UnlikeServlet extends HttpServlet{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5186745393297081101L;
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		if(user != null){
			...
			if(user.getEmail().equals(mail)){
				...
				int count = UserController.unlikeAction(mail, actionId);
				 resp.setContentType("text/html");
				 PrintWriter out = resp.getWriter();
				 out.print("<span style='color:black; margin-right:16px' class='badge badge-success'>"+Integer.toString(count)+"<a style='color:black;' class='vote' href='/unlike?id=[ID]&mail=[EMAIL]%>'><i class='icon-thumbs-up'></i>CONCORDAR</a></span>");
			}
